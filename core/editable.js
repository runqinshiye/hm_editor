...
( function() {
	...
	CKEDITOR.editable = CKEDITOR.tools.createClass( {
		...
		proto: {
			 ...
			 * @param {boolean} [doNotFocus] If true, 将不会执行 editor.focus() 命令
			 ...
			 */
			insertHtml: function (data, mode, range, doNotFocus) {
				var editor = this.editor;

				doNotFocus || editor.focus();
				editor.fire( 'saveSnapshot', {name: 'insertHtml'});
			...
			setup: function() {
				...
				this.attachListener( editor, 'getSnapshot', function getData( evt ) {
					evt.data = this.getData( 1 );
				}, this );

				this.attachListener( editor, 'afterSetData', function setData() {
					this.setData( editor.getData( 1 ) );
				}, this );

				this.attachListener( editor, 'loadSnapshot', function setData( evt ) {
				...
				this.attachListener( editor, 'insertHtml', function( evt ) {
					this.insertHtml( evt.data.dataValue, evt.data.mode, evt.data.range , evt.data.doNotFocus );
				}, this );
				...
				this.attachListener( editor, 'key', function handleDelete( evt ) {
					if ( editor.readOnly )
						return true;
				...
				if ( CKEDITOR.env.webkit ) {
					this.attachListener( editor, 'key', function handleDeleteInWebkit( evt ) {
						if ( editor.readOnly ) {
							return true;
						}
	...
	CKEDITOR.on( 'instanceLoaded', function( evt ) {
		...
		editor.on( 'selectionChange', function fixDom1( evt ) {
			if ( editor.readOnly )
				return;
	...
	insert = ( function() {
		...
		function insertDataIntoRange( that ) {
			...
			for ( ; nodeIndex < nodesData.length; nodeIndex++ ) {
                ...
                // modify By liwenjuan YUNHIS-24629  20220112
				// First not allowed node reached - start splitting original container
				if(nodeIndex == 0 && nodeData.name == 'p') {
					var element = new CKEDITOR.dom.element( 'span' );
					element.setHtml(nodeData.node.getHtml());
					nodeData.node = element;
					splittingContainer= 0;
				}else if(nodeData.firstNotAllowed) {
					splittingContainer = 1;
				}

				if(nodeIndex == nodesData.length - 1 && nodeData.name == 'p') {
					var element = new CKEDITOR.dom.element( 'span' );
					element.setHtml(nodeData.node.getHtml());
					nodeData.node = element;
					splittingContainer= 0;
	...
	function afterInsert( editable ) {
		...
		// call.
		setTimeout( function() {
			editor.fire( 'saveSnapshot', {name: 'afterInsert'});
		}, 0 );
	}
	...
	function mergeBlocksNonCollapsedSelection( editor, range, startPath ) {
		...
		// editor.fire( 'saveSnapshot' );  //modifyBy liwenjuan 为了处理修订模式下，段落选中删除撤销后，删除标识文本和删除的文本同时显示问题
